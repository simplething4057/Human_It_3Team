...
Netlify **Site configuration > Environment variables**에 아래 값을 먼저 설정해야 합니다.
...
| `VITE_API_URL` | (선택) API 엔드포인트 | `/.netlify/functions/api` |

> **중요**: Supabase 연결 시 포트 **6543**을 사용하는 Pooler 주소를 권장합니다.
...
## 3. Netlify 배포 설정
`netlify.toml`에 이미 모든 설정이 포함되어 있으나, 대시보드에서 다음을 확인하세요.
...
## 5. 자주 발생하는 이슈 해결 (Q&A)
- **로그인 시 400 에러**: `Lock to stop auto publishing`을 끄고 최신 코드가 배포되었는지 확인하세요. (바디 복구 미들웨어가 필요함)
